...
import com.neo.util.api.security.credential.BasicCredentials;
import com.neo.util.api.security.credential.BearerCredentials;
import com.neo.util.api.security.credential.TokenCredentials;
    ...
    protected Credential generate(String httpScheme, String httpHeader) {
        if (authenticationProvider.getSupportedAuthenticationSchemes().contains(httpScheme.toUpperCase())) {
            return switch (httpScheme.toUpperCase()) {
                ...
                default -> throw new ExternalRuntimeException(SecurityConstants.EX_UNSUPPORTED_AUTH_TYPE);
            };
        }
        throw new ExternalRuntimeException(SecurityConstants.EX_UNSUPPORTED_AUTH_TYPE);
    }
