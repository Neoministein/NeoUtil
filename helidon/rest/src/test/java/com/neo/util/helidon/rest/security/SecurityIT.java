...
class SecurityIT extends AbstractIntegrationTest {
    ...
    void secureResourceFailTest(WebTarget webTarget) {
        ...
        Assertions.assertEquals(SecurityConstants.EX_UNAUTHORIZED.getExceptionId(), responseBody.get("code").asString());
        Assertions.assertEquals(SecurityConstants.EX_UNAUTHORIZED.getFormat(), responseBody.get("message").asString());
    }
    ...
    void roleResourceFailureTest(WebTarget webTarget) {
        ...
        JsonNode responseBody = JsonUtil.fromJson(response.readEntity(String.class));
        Assertions.assertEquals(SecurityConstants.EX_FORBIDDEN.getExceptionId(), responseBody.get("code").asString());
        Assertions.assertEquals(SecurityConstants.EX_FORBIDDEN.getFormat(), responseBody.get("message").asString());
    }
}
